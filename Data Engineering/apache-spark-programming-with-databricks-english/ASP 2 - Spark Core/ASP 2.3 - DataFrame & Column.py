...
display(
events_df.select(events_df["device"]))
...
# MAGIC %scala
# MAGIC val dbpath = "dbfs:/mnt/dbacademy-datasets/apache-spark-programming-with-databricks/v03/ecommerce/events/events.delta"
# MAGIC events_df = spark.read.format("delta").load(dbpath)
# MAGIC events_df.select($"device")
...
rev_df = (events_df
         ...
         .selectExpr("user_id","CAST((ecommerce.purchase_revenue_in_usd * 100) AS INT)", "ecommerce.purchase_revenue_in_usd / ecommerce.total_item_quantity")
         #.sort("ecommerce.purchase_revenue_in_usd / ecommerce.total_item_quantity".desc())
        )
display(rev_df)        
...
events_df.dropDuplicates(["user_id"]).count()
